...
def get_fitness_2(spike_rate_x, spike_rate_control, order = 6, threshold = 0.6):
    ...
    adj_control_bursts = len([spike_rate_control[i] for i in control_bursts if spike_rate_control[i] >= max_val * threshold])
    adj_x_bursts = len([spike_rate_x[i] for i in x_bursts if spike_rate_x[i] >= max_val * threshold])
