...
class Evolution:
    def __init__(self, model_type, population_size, simulation_duration, reference_phenotype, parents_p, retained_adults_p, mutation_p):
    ...
    def generate_phenotype(self, individual):
        ...
        if self.model_type == "CA":
            phenotype = CellularAutomataModel.CellularAutomataModel(individual = individual, dimension = 30, duration= self.simulation_duration).run_simulation()
        ...
        fitness = Fitness.get_fitness(Data.get_spikes_pheno(phenotype, self.simulation_duration), self.reference_spikes)
